...
def test_db_probe_via_pod(
    ...
    Returns:
        ...
        - SKIPPED      : motor no soportado o ni python3 ni sh/nc disponibles
    """
    ...
    # ── Intento 2: fallback sh/nc (imágenes sin python3) ─────────────────────
    if py3_missing:
...
def export_results(data, output_format: str = "json", output_dir: str = "outcome"):
    ...
    if not EXPORT_MANAGER_AVAILABLE:
        ...
        if output_format == "json":
            filepath = output_path / f"deployment_validator_{ts}.json"
            with open(filepath, "w", encoding="utf-8") as f:
                json.dump({"generated_at": datetime.now().isoformat(), "data": data}, f, indent=2, default=str)
        elif output_format == "csv":
            filepath = output_path / f"deployment_validator_{ts}.csv"
    ...
    # Usar ExportManager
    manager = ExportManager("deployment_validator", "1.0.0")
