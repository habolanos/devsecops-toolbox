...
def main():
    ...
    # Rollback por revisión (TODO: implementar)
    if args.pipeline_id and args.to_revision:
        print(f"{Colors.YELLOW}⚠ Rollback por revisión aún no implementado{Colors.ENDC}")
        print(f"{Colors.CYAN}  Usa --backup-file para rollback desde backup local{Colors.ENDC}")
        return 1
