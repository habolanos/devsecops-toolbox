...
def get_git_commits(org: str, project: str, repo_id: str, branch: str,
    ...
    for c in commits:
        ...
        if not commit_id:
            c["changes"] = []
            continue
        commit_url = f"{base_url}/commits/{commit_id}/changes"
        try:
...
def get_git_tags(org: str, project: str, repo_id: str,
    ...
    for r in refs:
        ...
        try:
            ...
            if resp.status_code == 200:
                ann = resp.json()
                commit_sha = ann.get("taggedObject", {}).get("objectId", "")
                r["message"] = ann.get("message", "")
                r["createdBy"] = ann.get("taggedBy", {})
                if debug:
...
const releasePoints = timelineData
  ...
  .map(e => ({{
    ...
    status: e.status,
    stage: e.stage,
    isProdSuccess: e.status === 'succeeded' && e.stage === 'Production',
    user: e.user,
    changes: e.changes,
...
new Chart(ctx, {{
  ...
  data: {{
    datasets: [
      ...
      {{
        label: 'Exitoso (Production)',
        data: releasePoints.filter(p => p.status === 'succeeded' && p.isProdSuccess),
