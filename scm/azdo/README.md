...
|---|---|---|---|
| 2026-04-27 | 1.6.5 | Pestaña Charts en Excel de Health Score: 8 gráficos nativos Excel + 1 tabla heatmap. P1 Stacked Bar composición score, P2 Pie distribución ratings, P3 Grouped Bar DORA CI vs CD, P5 Scatter score vs uso, P6 Treemap tecnologías (combo bar+line), P7 Pareto tecnologías críticas (bar+%acumulado), P8 Tendencia histórica (desde cache), Heatmap Technology Status vs Rating con colores condicionales. Fix normalize_org en 3 scripts. Flag --run-inventory con spinner dinámico. Grupo health en tools.py. make_dist.ps1 incluye .cache/ en ZIP para gráficos | `azdo_pipeline_health_score.py`, `cicd_inventory_ci_detailed.py`, `cicd_inventory_cd_detailed.py`, `tools.py`, `README.md`, `make_dist.ps1` |
