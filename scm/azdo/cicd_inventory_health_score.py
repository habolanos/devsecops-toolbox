...
def export_results(data, output_format: str = "json", output_dir: str = "outcome"):
    ...
    if not EXPORT_MANAGER_AVAILABLE:
        ...
        if output_format == "json":
            filepath = output_path / f"cicd_inventory_health_score_{ts}.json"
            with open(filepath, "w", encoding="utf-8") as f:
                json.dump({"generated_at": datetime.now().isoformat(), "data": data}, f, indent=2, default=str)
        elif output_format == "csv":
            filepath = output_path / f"cicd_inventory_health_score_{ts}.csv"
    ...
    # Usar ExportManager
    manager = ExportManager("cicd_inventory_health_score", "1.0.0")
